...
if hum and horse:
    ...
    # 1) STOP the AI so it can't re-engage movement
    ...
    print("  stop_movement:", call(ai, "stop_movement"))
    if brain and "ERR" not in str(brain):
        print("  brain.stop_logic:", call(brain, "stop_logic", "stowed"))
    # 2) freeze movement + collision, then attach
    ...
    print("after: loc =", hum.get_actor_location(), "(z should be ~horse height, not -4000)")
    print(">> CHECK: is it now seated on the horse + STABLE (not falling/blinking)? full body?")
