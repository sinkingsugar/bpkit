...
if hum and horse:
    ...
    mesh.set_animation_mode(unreal.AnimationMode.ANIMATION_BLUEPRINT)  # anim instance MUST be running for slots
    ...
    r = call(anim, "play_slot_animation_as_dynamic_montage", idle, SLOT, 0.25, 0.25, 0.2, 10)  # 10 loops @0.2x ~ 200s
    print("play idle on '%s' ->" % SLOT, r)
    print("is_slot_active('%s'):" % SLOT, call(anim, "is_slot_active", SLOT))
    print("slot weight:", call(anim, "blueprint_get_slot_montage_local_weight", SLOT))
    print(">> ~200s window: SEATED full-body now on BOTH screens?")
