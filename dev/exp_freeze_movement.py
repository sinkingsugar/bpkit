...
if hum and horse:
    ...
    cm = hum.get_character_movement()
    # FULL freeze of CharacterMovement so it can't fall when attached high
    call(cm, "set_movement_mode", unreal.MovementMode.MOVE_NONE, 0)
    try: cm.set_editor_property("gravity_scale", 0.0)
    except Exception as e: print("gravity ERR", e)
    call(cm, "stop_movement_immediately")
    cm.set_component_tick_enabled(False)   # don't process movement at all
    ...
    print(">> watch: stable on the horse + seated now?")
